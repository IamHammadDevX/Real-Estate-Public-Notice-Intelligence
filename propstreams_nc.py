...
def main():
    ...
    db_table_name = 'NcPub'
    propstream_table = 'propstream_nc'
    truthfinder_table = 'truthfinder_nc'
    ...
    # Determine data source: USE_DB=1 to use MySQL; otherwise read input CSV named input_propstream_nc.csv
    ...
    if use_db:
        ...
        input_file = os.path.join(os.getcwd(), 'input_propstream_nc.csv')
        db_data = _read_pub_csv(input_file)

        # If manual input file is missing or nearly empty, auto-load from NcPub scraped exports
        if len(db_data) <= 1:
            exports_dir = os.path.join(os.getcwd(), 'exports')
            # Prefer the full NcPub.csv; fall back to NcPub_fallback_*.csv files
            candidate_files = sorted(_glob.glob(os.path.join(exports_dir, 'NcPub.csv')))
            candidate_files += sorted(_glob.glob(os.path.join(exports_dir, 'NcPub_fallback_*.csv')))
            ...
            for gf in candidate_files:
                for r in _read_pub_csv(gf):
                    if r['Id'] not in seen_ids:
                        ...
                        seen_ids.add(r['Id'])
            if db_data:
                util.print_log(f"Auto-loaded {len(db_data)} rows from NcPub exports")
            else:
    ...
    if results:
        ...
        try:
            ...
            xlsx_path = os.path.join(out_dir, f'propstream_nc_{ts}.xlsx')
